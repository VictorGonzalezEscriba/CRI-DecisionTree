...
"""
    def is_decision(self):
        if not self.root:
            if self.inner_edge.true == 0 and self.inner_edge.false == 0:
                true = self.inner_edge.true
                false = self.inner_edge.false
                if true > false:
                    self.decision = '<=50K'
                ...
                if self.inner_edge.true == 0:
                    self.decision = '>50K'
                    self.attribute = '>50K'
                    return True
                elif self.inner_edge.false == 0:
                    self.decision = '<=50K'
                    self.attribute = '<=50K'
                    return True
        return False
...
class ID3:
    ...
    def calculate_gain_id3(self, att_entropy, node):
    ...
    def choose_winner(self, tf_array, node=None, edge=None):
        ...
        if len(entropy_array) != 0:
            for entropy in entropy_array:
                gain_array.append(self.calculate_gain_id3(att_entropy=entropy, node=node))
...
class C45:
    def __init__(self, data):
        ...
        def calculate_split_info(self, edge):
            return 0

        def calculate_gain_ratio(self, split):
            return 0